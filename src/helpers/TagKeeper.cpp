...
bool CTagKeeper::removeDynamicTag(const std::string& s) {
    // rule tag effects may carry the apply prefix (+tag) while the stored
    // dynamic tag never does, so strip it for the lookup. A remove prefix
    // is left alone to preserve the current no-op behavior for it.
    const std::string_view base = s.starts_with("+") ? std::string_view{s}.substr(1) : std::string_view{s};
    return std::erase_if(m_tags, [&base](const auto& tag) { return tag == std::format("{}*", base); });
}
